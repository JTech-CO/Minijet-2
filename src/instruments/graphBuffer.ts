import { InstrumentChannelId, type GraphChannel, type GraphSample, type TelemetryGraphBuffer } from "./types.ts";

export const GRAPH_CHANNELS: GraphChannel[] = [
...
export function createTelemetryGraphBuffer(maxSamples = 600): TelemetryGraphBuffer {
  return {
    maxSamples,
...
export function getGraphSeries(
  buffer: TelemetryGraphBuffer,
  channelId: InstrumentChannelId
): Array<{ timeS: number; value: number }> {
...
export function latestGraphSample(buffer: TelemetryGraphBuffer): GraphSample | null {
