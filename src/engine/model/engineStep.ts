import { buildEcuStatus, createIdleEcuStatus } from "../ecu/diagnostics.ts";
import { applyProtections } from "../ecu/protections.ts";
import { scheduleEcuCommand } from "../ecu/schedules.ts";
import { updateStartSequence } from "../ecu/startSequence.ts";
import { computeAtmosphere } from "./atmosphere.ts";
import { updateCombustor } from "./combustor.ts";
import { computeCompressor } from "./compressor.ts";
import { computeElectricalSystem, updateFuelSystem } from "./fuelSystem.ts";
import { computeNozzle } from "./nozzle.ts";
import { updateSensors } from "./sensors.ts";
import { updateSpool } from "./spool.ts";
import { updateThermalState } from "./thermal.ts";
import { computeTurbineTorque } from "./turbine.ts";
import type { EngineInputs, EnginePerformanceTrace, EngineSpec, EngineState, EngineTelemetry } from "./types.ts";
import { EngineMode } from "./types.ts";
import { clamp, rpmToRadPerSec } from "./units.ts";

function createZeroPerformanceTrace(): EnginePerformanceTrace {
  return {
...
export function createInitialEngineState(spec: EngineSpec, ambientTemperatureC = 15): EngineState {
  return {
    mode: EngineMode.Off,
    modeElapsedS: 0,
...
export function toTelemetry(state: EngineState, spec: EngineSpec): EngineTelemetry {
  return {
    mode: state.mode,
    faults: [...state.faults],
