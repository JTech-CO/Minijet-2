import type { EcuCommand, EngineInputs, EngineSpec, EngineState } from "./types.ts";
import { approachExp, clamp, smoothstep } from "./units.ts";

export function computeBatteryAuthority(voltageV: number, spec: EngineSpec): number {
...
export function updateFuelSystem(
  ...
  spec: EngineSpec,
  dtS: number
): Pick<EngineState, "fuelFlowKgPerS" | "commandedFuelFlowKgPerS" | "pumpPressureBar" | "fuelRemainingKg"> {
