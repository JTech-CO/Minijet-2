import type { EcuCommand, EcuStatus, EngineInputs, EngineSpec, EngineState } from "../model/types.ts";
import { StartBlockReason } from "../model/types.ts";
import { clamp } from "../model/units.ts";
...
): Pick<EcuStatus, "startPermissive" | "startBlockReason"> {
...
export function evaluateLightOffReady(state: EngineState, spec: EngineSpec): boolean {
  const mixtureStable =
...
export function createIdleEcuStatus(): EcuStatus {
  return {
