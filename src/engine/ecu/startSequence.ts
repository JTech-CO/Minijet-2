import { evaluateStartReadiness } from "./diagnostics.ts";
import type { EngineInputs, EngineSpec, EngineState, FaultCode as FaultCodeType } from "../model/types.ts";
...
export function updateStartSequence(
  ...
  const addedFaults: FaultCodeType[] = [];
  ...
  const transitionTo = (nextMode: EngineState["mode"]) => {
    if (nextMode !== mode) {
      mode = nextMode;
      modeElapsedS = 0;
    }
