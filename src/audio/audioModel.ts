...
import { EngineMode, FaultCode } from "../engine/index.ts";
import { clamp } from "../engine/model/units.ts";
import { AudioLayerId, type AudioCue, type AudioLayerState, type EngineAudioState } from "./types.ts";
...
function faultCue(fault: FaultCode): AudioCue {
  switch (fault) {
...
export function createEngineAudioState(
  ...
  const layers: Record<AudioLayerId, AudioLayerState> = {
